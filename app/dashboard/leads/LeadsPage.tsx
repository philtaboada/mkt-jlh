...
import type { LeadFormInput } from '@/lib/schemas/leadSchemas';
...
export function LeadsPageClient() {
  ...
  //const productsExistsQuery = useVerifyProducts(editing?.id || '');
  //.log('Products exists query data:', productsExistsQuery.//);
  const onSendToDeals = async (type: string, payload: any) => {
    try {
      // Crear el prospecto según el tipo
      ...
      // Actualizar el estado del lead a 'deals' si tenemos el leadId
      ...
      <EntityDialog
        ...
        content={(onClose) => (
          <LeadForm
            ...
            onSubmit={async (data) => {
              console.log('Submitting lead form with data:', data);
              if (editing) {
