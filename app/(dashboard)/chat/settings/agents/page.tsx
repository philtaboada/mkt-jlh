            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="flex gap-2 pt-4">
                  ...
                  >
                    {createAgentMutation.isPending ? 'Creando...' : 'Agregar Agente'}
                  </Button>
                </div>
              </div>
            </DialogContent>
        ...
        <div className="flex gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">
              {agents.filter((a: Agent) => a.status === 'online').length} en línea
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Circle className="w-2 h-2 fill-amber-500 text-amber-500" />
            <span className="text-muted-foreground">
              {agents.filter((a: Agent) => a.status === 'busy').length} ocupados
            </span>
          </div>
          <div className="flex items-center gap-2">
            <Circle className="w-2 h-2 fill-muted-foreground text-muted-foreground" />
            <span className="text-muted-foreground">
              {agents.filter((a: Agent) => a.status === 'offline').length} desconectados
            </span>
          </div>
        </div>
      </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant="secondary"
                            className={cn('text-xs', roleColors[agent.role])}
                          >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <SelectTrigger className="w-36 h-8 text-xs">
                            <div className="flex items-center gap-2">
                              <Circle
                                className={cn('w-2 h-2 fill-current', statusColors[agent.status])}
                              />
                              <SelectValue />
                            </div>
                          </SelectTrigger>
