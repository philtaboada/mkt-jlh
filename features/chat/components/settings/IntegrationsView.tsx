...
export function IntegrationsView() {
  ...
  const handleCreateChannel = () => {
    ...
    createChannelMutation.mutate(
      { name: channelName, type: selectedType, config: {} },
      {
        onSuccess: () => {
  ...
  const handleToggleStatus = (channel: Channel) => {
    ...
    updateChannelMutation.mutate(
      { id: channel.id, input: { status: newStatus } },
      {
        onSuccess: () => {
          toast.success(
            newStatus === 'active'
  ...
  const handleValidateChannel = async (channel: Channel) => {
    ...
    try {
      if (channel.type === 'website') {
        ...
        if (response.ok) {
          updateChannelMutation.mutate(
            { id: channel.id, input: { status: 'active' } },
            { onSuccess: () => toast.success('Canal validado y activado correctamente') }
          );
        } else {
        ...
        updateChannelMutation.mutate(
          { id: channel.id, input: { status: 'active' } },
          { onSuccess: () => toast.success('Canal activado correctamente') }
        );
      }
    } catch (error) {
      ...
      <div className="flex-1 p-6 overflow-auto">
        <div className="max-w-4xl mx-auto space-y-6">
          {isLoading ? (
            ...
            <IntegrationsEmptyState onCreateClick={() => setCreateDialogOpen(true)} />
          ) : (
            <div className="space-y-4">
              {channels.map((channel: Channel) => (
                <ChannelCard
                  ...
                  isValidating={validatingChannel === channel.id}
                />
              ))}
            </div>
          )}
...
interface IntegrationsHeaderProps {
  ...
  setSelectedType: (type: ChannelType | null) => void;
  channelName: string;
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Gestiona tus canales de comunicación</p>
            </div>
          </div>
        </div>
...
function IntegrationsEmptyState({ onCreateClick }: { onCreateClick: () => void }) {
  return (
    <Card className="bg-card">
      <CardContent className="py-12 text-center">
        <MessageSquare className="w-12 h-12 mx-auto text-muted-foreground mb-4" />
        <h3 className="text-lg font-medium text-foreground mb-2">No hay canales configurados</h3>
...
interface ChannelCardProps {
  channel: Channel;
  getChannelIcon: (type: ChannelType) => React.ComponentType<{ className?: string }>;
  getChannelColor: (type: ChannelType) => string;
  onDelete: (id: string) => void;
  onToggleStatus: (channel: Channel) => void;
  onValidate: (channel: Channel) => void;
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem asChild>
                  <Link href={`/chat/settings/integrations/${channel.type}/${channel.id}`}>
                ...
                {channel.status === 'pending' && (
                  <DropdownMenuItem onClick={() => onValidate(channel)} disabled={isValidating}>
                    {isValidating ? (
                ...
                {channel.status !== 'pending' && (
                  <DropdownMenuItem onClick={() => onToggleStatus(channel)}>
                ...
                <DropdownMenuItem className="text-destructive" onClick={() => onDelete(channel.id)}>
      ...
      {channel.type === 'whatsapp' && (
        <CardContent className="pt-0">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>
              {(channel.config as { phone_number?: string })?.phone_number ||
...
function AvailableChannels({ channelTypes, channels, onSelect }: AvailableChannelsProps) {
  return (
    <div className="pt-6">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {channelTypes.map((channel) => {
          const Icon = channel.icon;
          const existingCount = channels.filter((c) => c.type === channel.type).length;
...
const channelTypes_export = channelTypes;
